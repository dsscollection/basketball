...
\if0\blind
{
  \title{\bf Modeling Offensive Player Movement in Professional Basketball}
  \author{Steven Wu\thanks{
    The authors gratefully acknowledge Benjamin S. Baumer, Sean J. Taylor, and Jennifer Bryan for their gracious time in reviewing this article.}\hspace{.2cm}\\
    Department of Statistics and Actuarial Science, Simon Fraser University\\
    and \\
    Luke Bornn \\
    Department of Statistics and Actuarial Science, Simon Fraser University\\Sacramento Kings}
  \maketitle
} \fi
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf Modeling Offensive Player Movement in Professional Basketball}
\end{center}
  \medskip
} \fi
...
\noindent%
{\it Keywords:}  sports statistics; data science; data visualization
\vfill
...
\label{column_table}
\begin{tabular}{ p{0.125\linewidth} p{0.1\linewidth} p{0.775\linewidth} }
...
%
\node (moments2) [data, right of=moments, xshift=4cm] {moments};
\node (dir_of_play) [data, below of=moments2, yshift=-0.75cm] {dir\_of\_play};
...
%
\node (off_moments) [data, right of=dir_of_play, xshift=5.5cm] {offensive\_moments};
...
\node (off_moments2) [data, below of=p_id, yshift=-0.75cm] {offensive\_moments};
\node (team_id2) [constant, below of=off_moments2, yshift=-0.75cm] {team\_id};
%
\node (player_df) [data, right of=off_moments2, xshift=5.5cm] {player\_df};
...
%
\node (flipped_df) [data, right of=player_df, xshift=3.5cm] {flipped\_df};
...
%
\node (filtered_df) [data, right of=flipped_df2, xshift=3.75cm] {df};
\draw [arrow] (flipped_df2) -- node [anchor=center, below]{filter\_for\_on\_or\_off\_ball} (filtered_df);
%
\node (emp_etas) [data, right of=filtered_df, xshift=3cm] {empirical\_etas};
...
%
\node (regression_outputs) [data, left of=regression_inputs, xshift=-4.5cm] {regression\_outputs};
